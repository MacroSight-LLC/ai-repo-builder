...
from cuga.backend.tools_env.registry.config.config_loader import Auth, Service, ServiceConfig
...
from cuga.backend.utils.consts import LOCAL_ORCHESTRATE_URL, LOCAL_TRM_URL, ServiceType
...
class MCPManager:
        ...
        for schema_type in ["allOf", "anyOf", "oneOf"]:
            if schema_type in modified_schema and isinstance(modified_schema[schema_type], list):
                ...
                for sub_schema in modified_schema[schema_type]:
                    modified_sub_schema = self._remove_parameter_from_body_schema(
                        sub_schema, parameter_name
                    )
                                ...
                                if "parameters" in operation:
                                    ...
                                    for param in operation["parameters"]:
                                        ...
                                        if (
                                            param.get("in") == "query"
                                            and param.get("name")
                                            in query_override_map[operation_id]
                                        ):
    ...
    def get_all_apis(self, include_response_schema=False):
        ...
        tools = {
            app: self.get_apis_for_application(app, include_response_schema) for app in app_names
        }
        return tools
    ...
    def get_apis_for_application(self, app_name, include_response_schema=False):
        ...
        if is_trm_app:
            ...
            for s in self.schemas[app_name]:
                ...
                if include_response_schema:
                    s_copy["response_schemas"] = self._convert_trm_response_schema(
                        s.get("output_schema", {})
                    )
                else:
        ...
        is_mcp_server = app_name in self.mcp_clients or (
            app_name in self.schema_urls
            and self.schema_urls[app_name].type == ServiceType.MCP_SERVER
        )
            ...
            for name, config in mcp_servers:
                ...
                try:
                    ...
                    if hasattr(transport, "_process") and hasattr(transport._process, "stderr"):
                        try:
                            stderr_output = (
                                transport._process.stderr.read()
    ...
    def _create_transport(self, name: str, config: ServiceConfig):
        ...
        if transport_type == "stdio":
            ...
            # Merge the full parent environment with any config-level overrides.
            # The MCP SDK's stdio_client only inherits 6 allowlisted vars by
            # default (HOME, PATH, SHELL, TERM, USER, LOGNAME), which strips
            # API keys etc.  Passing the full os.environ ensures child processes
            # see GITHUB_PERSONAL_ACCESS_TOKEN, BRAVE_API_KEY, and friends.
            import os as _os

            stdio_env = {**_os.environ, **(config.env or {})}
            return StdioTransport(command=config.command, args=config.args or [], env=stdio_env)
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(
                        ...
                        if response.status == 200:
                            ...
                            structured_content = (
                                result.get("structured_content", None)
        ...
        for name, config in services:
            try:
                ...
                has_body_overrides = any(
                    override.drop_request_body_parameters
                    for override in (config.api_overrides or [])
                )
                ...
                if (
                    config.include
                    or config.api_overrides
                    or has_body_overrides
                    or has_query_overrides
                ):
